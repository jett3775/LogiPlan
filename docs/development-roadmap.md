...
6. （既有环境不稳定，未定位根因）Gate 1 浏览器阶段偶发 Windows 原生崩溃 `3221226505`（`0xC0000005`）。2026-09-21、2026-09-22、2026-09-24 三轮均有记录，累计样本中崩溃率约 20%（2026-09-24 两批次 10 次执行中 2 次）。特征：发生在浏览器阶段启动边界、无用例输出或 0ms 即失败、零断言失败、同一批用例在其余执行中通过。已排除用户浏览器负载与磁盘空间；**未定位根因**（未做 WER/崩溃转储级排查）。影响：无法取得「连续 5/5」，Gate 1 不能记为稳定通过。后续任务：如需消除，应采集 Windows 事件日志/WER 崩溃转储确认崩溃进程（Playwright worker 与浏览器进程需区分），再评估浏览器启动参数类缓解措施——**该类改动超出本轮允许范围，须先取得用户批准**。**（2026-09-25 更新）只读取证已执行，根因无法定位**：经用户单独授权检查 Windows 事件日志与 WER 报告——Application 日志 Id=1000/1001 共 400 条（覆盖 2026-09-18 → 09-25）中匹配 `c0000005` 的为 **0 条**；`ReportArchive` 中无 node / chrome / firefox / playwright 报告；Playwright 的 Chromium profile 下无 Crashpad 报告；WER 未被禁用（`Disabled` 与 `LoggingDisabled` 均未设置）。09-21 → 09-25 窗口内唯一提到本项目相关进程的事件是 2026-09-22 10:06:55 的 `RADAR_PRE_LEAK_64`（`node.exe` 24.15.0.0），属资源泄漏预警启发式而非崩溃。机器上确有一份 2026-09-21 的浏览器崩溃转储（`EXCEPTION_ACCESS_VIOLATION_READ`、`crash_address 0x0`），但经 `ProfileDirectory`（`eqnm8cyw.default-release`）、`URL`（`chat.deepseek.com`）、模块（含用户输入法 `weasel.dll`）与会话时长 9886 s 判定为**用户自己的 Firefox**，与 Playwright 捆绑构建 `firefox-1538` 无关，不得计入本项目证据。**结论：Windows 层面无任何可归因记录，在批准的只读范围内无法定位根因。** 据此按 **D-188** 冻结：CI `ubuntu-latest` 为 Gate 1 稳定性的权威证据来源（已连续 6 次全绿），本地 Windows 记录为已接受的环境限制并附明确重试政策。**该崩溃的根因仍未关闭**；如需消除，须另行批准启用崩溃转储采集（例如为 Playwright 的浏览器进程配置 `LocalDumps`）。
...
- **T1 本地正式复验（2026-09-25，用户终端，全部通过）**：`pnpm test:db-integration` 四条腿 `fail=0`，且 **`skipped=0` 与显式声明的平台门控跳过数完全一致**（Windows 上声明值即为 0）——这是从 Windows 侧对 T3 平台判断的独立验证。`pnpm verify:gate1:isolated` 退出码 0，且**本次是完整且干净的 Gate 1，未命中 `3221226505`**。分项：迁移 0001—0003 → V1 → 0004—0010 → V2 候选 `VALIDATED` → 显式激活 → 重复发布幂等；结构/精度/三角色权限、不可变发布升级、6 条查询计划 `temp_written_blocks` 全 0；生产构建完整通过（`Finalizing page optimization in 9ms`）；快照集成 28/28；Chromium 双视口基础 10 passed / 22 skipped；Chromium 双视口历史证据 22/22；Firefox 核心冒烟 3/3；并发 5 × 100 热查询 p50 28.753 ms / p95 50.459 ms / p99 56.447 ms（P95 预算 1 s）；运行后隔离容器、卷、网络全部移除。`pnpm lint`、`pnpm typecheck`（4 个 workspace）、`pnpm test`（14 文件 113 passed / 11 skipped）均退出码 0。
- **T6 只读取证与 G2 冻结决策（2026-09-25，经用户单独授权）**：取证结论见遗留项 6 的「2026-09-25 更新」——Windows 层面无任何可归因记录，根因无法定位。据此写入 **D-188**：Gate 1 稳定性的权威证据来源定为 CI `ubuntu-latest`，本地 Windows 记为已记录、已接受的环境限制并附重试政策。**G2 达成。**
- **第二窗口 P1—P6（只读准备）全部完成**：P1 冻结验证（runbook §1.1）、P2 部署前清单（§7，14 项）、P3 平台配额核对（§8）、P4 环境变量分层（§5.1）、P5 GitHub 安全治理（§10）、P6 Neon 隔离性（§9）。**P5 步骤 1「启用秘密扫描」经用户授权已执行**：`secret_scanning` 与 `secret_scanning_push_protection` 已 enabled，`GET /secret-scanning/alerts` 返回 **0 条**（生效证据）；`non_provider_patterns` 与 `validity_checks` 因可用性限制未生效。
- **三项授权收尾**：① 启用秘密扫描（见上）；② `gate1.yml:88` 的产物凭据扫描补齐三个 `NEON_*_PASSWORD`，按角色成组排列；③ 实现 D-177 的 Zod 环境校验——新增 `packages/db/src/runtime-env.ts` 的 `readWebRuntimeDatabaseUrl()`（校验协议且**用户名必须为 `app_reader`**；未配置或空串返回 `null` 以保持「未配置即不健康」的既有构建行为；失败抛错且**不回显连接串**），改造 `apps/web` 的三个调用点，9 项测试、覆盖率 100%，并加入 `vitest.config.ts` 的 `coverage.include`。实现放在 `packages/db` 而非 `apps/web`，因为该包已依赖 zod 4.4.3，可**完全避免改动 `pnpm-lock.yaml`**。
- **公开测试环境发布门改设计（本轮最重要的结论）**：查证 Vercel 官方文档后确认，冻结要求「合并 `main` 不自动发布 + 人工批准 + 绑定具体提交 SHA」可**完全由 Vercel 原生 staged production 满足**——关闭 Settings → Environments → Production → Branch Tracking 的「Auto-assign Custom Production Domains」后，推送到生产分支只产生 `Staged` 部署（不对外服务），须人工 Promote 才变 `Current`，且 **promote 不触发重建**。因此 runbook §10.5 原设想的「自建 GitHub Actions 受保护工作流」应改为原生方案：后者需把 Vercel token 放进 GitHub secrets（**新的凭据面**），且 GitHub Environment 保护规则**管不到** Vercel 自己的 Git 集成部署。**附带效果：关闭 auto-assign 后合并 `main` 变得安全**（只产生不对外服务的 Staged 构建）。
- **远程部署现状（只读核查）**：GitHub 记录部署共 15 次（Preview 14 + **Production 1**）；Production 停留在 2026-09-10 的 `a63a43c`（= `origin/main` 当前 tip），**落后本分支 34 个提交**；Vercel 项目已存在且与仓库 Git 集成连通（每次推送自动生成 Preview，最新一次对应 `c27d5fe`，state = success）；`Production` 环境的 `protection_rules` 为 **0**。
- **仍未执行**：`3221226505` 的根因消除（D-188 明确其根因未关闭，如需消除须另行批准启用崩溃转储采集）；写入模式前的工具 SHA 重新冻结与批准（属第一窗口计划 P1）；公开环境部署与数据激活（仍各自需要显式授权）。
...
合并 `main` 不自动发布公开测试环境；受保护 Production 工作流必须由人工批准，并绑定已通过检查的具体提交 SHA、迁移清单和数据包校验和。（**实现机制见 D-189**：改用 Vercel 原生 staged production 实现，不再自建 GitHub Actions 工作流；理由是该方案零新增凭据面，且 GitHub Environment 的保护规则管不到 Vercel 自己的 Git 集成部署。）
