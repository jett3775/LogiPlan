-- Make publisher-side materialization idempotent while keeping the snapshot
-- table inaccessible for direct publisher reads and writes.
...
BEGIN
  ...
  END IF;
  SELECT s.result INTO saved_result
  FROM logiplan.evidence_snapshot AS s
  WHERE s.data_release_id = release_id
    AND s.result->>'result_id' = query_result->>'result_id'
  ORDER BY s.created_at DESC
  LIMIT 1;
  IF FOUND THEN RETURN saved_result; END IF;
