...
const previewDir = path.join(
  workspace,
  ".tmp",
  "spreadsheet-build-019ff4dd",
  "previews-labor-split",
);
...
if (process.argv[2] === "--inspect-existing") {
  ...
  console.log(
    JSON.stringify({ values: checkRange.values, formulas: checkRange.formulas }, null, 2),
  );
  ...
  await fs.writeFile(
    path.join(path.dirname(inputPath), "Checks-imported.png"),
...
function setFormulaColumn(sheet, col, startRow, formulas) {
  ...
  sheet.getRange(`${col}${startRow}:${col}${startRow + formulas.length - 1}`).formulas =
...
// Summary
{
  ...
  titleBand(
    sheet,
    "LogiPlan AI｜2026 物流财务演示数据",
    "虚构求职作品集数据｜Budget 2026 V1｜Actual 截至 2026-08｜Forecast 2026-08 V1｜报告币种 CNY",
    "S",
  );

  const cards = [
    {
      label: "年度 Budget 物流总成本",
      labelRange: "A4:D4",
      valueRange: "A5:D7",
      formula: "=SUMIF('Monthly_Summary'!$A$5:$A$52,\"BUDGET\",'Monthly_Summary'!$I$5:$I$52)",
      fill: COLORS.sky,
      color: COLORS.blue,
    },
    {
      label: "Latest Outlook 物流总成本",
      labelRange: "F4:I4",
      valueRange: "F5:I7",
      formula:
        "=SUMIF('Monthly_Summary'!$A$5:$A$52,\"LATEST_OUTLOOK\",'Monthly_Summary'!$I$5:$I$52)",
      fill: COLORS.sky,
      color: COLORS.blue,
    },
    {
    ...
    {
      label: "Cost Saving 9—12 月节省",
      labelRange: "P4:S4",
      valueRange: "P5:S7",
      formula:
        "=SUMIF('Monthly_Summary'!$A$5:$A$52,\"FORECAST\",'Monthly_Summary'!$I$5:$I$52)-SUMIF('Monthly_Summary'!$A$5:$A$52,\"SCENARIO_COST_SAVING\",'Monthly_Summary'!$I$5:$I$52)",
  ...
  for (const card of cards) {
    ...
    sheet.getRange(card.labelRange).format = {
      fill: card.fill,
    ...
    sheet.getRange(card.valueRange).format = {
      fill: card.fill,
      font: { bold: true, color: card.color, size: 17, name: "Aptos Display" },
      horizontalAlignment: "center",
      verticalAlignment: "center",
      borders: { preset: "outside", style: "thin", color: COLORS.border },
  ...
  sheet.getRange("A9").values = [
    [
      "核心结论：2026 年 8 月英国为最大单月不利异常。促销拉动订单量，同时德国仓至英国的空运和 Carrier C 份额上升；五因素中结构影响最大、量影响其次。Cost Saving 仅在现有线路内调整，准时率约下降 0.19 个百分点并触发软提示，承运能力仍未验证。",
    ],
  ];
  sheet.getRange("A9:S10").format = {
  ...
  const monthlyRows = sourceRowMap(
    data.monthly_summary,
    (row) => `${row.series_id}|${row.month_id}`,
  );
  const trendValues = [];
  ...
  for (let month = 1; month <= 12; month += 1) {
    ...
    trendFormulasLatest.push([
      `='Monthly_Summary'!I${monthlyRows.get(`LATEST_OUTLOOK|${monthId}`)}`,
    ]);
  }
  ...
  writeMatrix(
    ...
    31,
    5,
    factors.map(([label]) => [label, null]),
  );
  for (let index = 0; index < factors.length; index += 1) {
    const factor = factors[index][1];
    sheet.getRange(`F${31 + index}`).formulas = [
      [
        `=SUMIFS('Attribution'!$G$5:$G$804,'Attribution'!$A$5:$A$804,\"ACTUAL_VS_BUDGET\",'Attribution'!$B$5:$B$804,DATE(2026,8,1),'Attribution'!$C$5:$C$804,\"CHAIN\",'Attribution'!$D$5:$D$804,\"${factor}\",'Attribution'!$E$5:$E$804,\"GB\")`,
      ],
    ];
  }
  ...
  for (let month = 9; month <= 12; month += 1) {
    ...
    sheet.getRange(`I${row}`).formulas = [
      [`='Monthly_Summary'!I${monthlyRows.get(`FORECAST|${monthId}`)}`],
    ];
    sheet.getRange(`J${row}`).formulas = [
      [`='Monthly_Summary'!I${monthlyRows.get(`SCENARIO_COST_SAVING|${monthId}`)}`],
    ];
...
// Assumptions
{
  ...
  titleBand(
    sheet,
    "模型假设与版本边界",
    "蓝色为冻结规则；黄色为本次虚构数据的演示参数。任何新业务定义应先进入 decisions.md。",
    "D",
  );
  const rows = [
    ["类别", "项目", "取值", "说明"],
    ...
    [
      "冻结规则",
      "最新结账月",
      data.metadata.latest_closed_month,
      "1—8 月 Actual；9—12 月 Forecast",
    ],
    [
      "冻结规则",
      "归因顺序",
      data.metadata.attribution_sequence.join(" → "),
      "Shapley 32 个组合仅作后台参考",
    ],
    [
      "冻结规则",
      "变动成本范围",
      "6 类",
      "基础运费、燃油附加费、一线作业弹性人工、包装、退货物流、异常配送",
    ],
    ["冻结规则", "固定成本范围", "4 类", "仓租、一线作业基础人工、仓库管理人工、系统费用单独展示"],
    [
      "冻结规则",
      "人工成本口径",
      data.metadata.cost_model_revision,
      "计件、加班和临时用工为变动成本；一线底薪及管理人工为固定成本",
    ],
    ...
    [
      ...
      "8 月英国背景",
      "UK promotion",
      "管理动作注释：提高德国仓至英国空运和 Carrier C 份额",
    ],
    ["演示参数", "税费", "不含 VAT/销售税", "正常清关在基础运费，异常清关在异常配送费"],
    [
      "技术规则",
      "高精度权威数据",
      "Decimal / numeric(50,24)",
      "工作簿公式用于审阅；JSON 保留高精度计算结果",
    ],
  ];
...
// Route master
{
  ...
  const rows = data.dimensions.routes.map((row) => [
    ...
    new Date(`${row.active_from}T00:00:00Z`),
    row.active_to ? new Date(`${row.active_to}T00:00:00Z`) : null,
    row.capacity_status,
    { DE: "德国", FR: "法国", GB: "英国" }[row.destination_country_id],
...
const serviceRateMap = new Map(
  data.service_facts.map((row) => [
    `${row.version_id}|${row.month_id}|${row.route_id}`,
    asNumber(row.on_time_rate),
  ]),
);
const modelRowMap = new Map();
{
  ...
  titleBand(
    sheet,
    "线路驱动与公式计算",
    "浅黄色列为输入；浅蓝色列为公式结果。工作簿公式用于审阅，权威高精度结果保存在生成 JSON。",
    "BG",
  );
  const headers = [
    "Version",
    ...
    "GBP/CNY",
    "Transport FX",
    "Base Freight Original",
    "Fuel Original",
    "Base Freight CNY",
    "Fuel CNY",
    "Variable Labor Currency",
    "Variable Labor Unit Price",
    "Variable Labor FX",
    "Variable Labor Original",
    "Variable Labor CNY",
    "Packaging Currency",
  ...
  const rows = modelCalcRows.map((row, index) => {
    ...
    return [
      ...
      row.carrier_id,
      row.transport_mode_id,
      asNumber(row.total_company_orders),
      asNumber(row.destination_order_share),
      asNumber(row.warehouse_order_share),
      asNumber(row.route_package_share),
      null,
      asNumber(row.average_packages_per_order),
      null,
      asNumber(row.average_chargeable_weight_per_package_kg),
      null,
      asNumber(row.return_rate),
      null,
      asNumber(row.billable_events_per_package),
      null,
      row.transport_currency,
      asNumber(row.effective_base_rate_per_kg),
      row.fuel_charge_basis,
      asNumber(row.fuel_rate_or_unit_price),
      asNumber(row.eur_cny_rate),
      asNumber(row.gbp_cny_rate),
      null,
      null,
      null,
      null,
      ...
      asNumber(row.frontline_variable_labor_unit_price),
      null,
      null,
      null,
      row.packaging_currency,
      asNumber(row.packaging_unit_price),
      null,
      null,
      null,
      row.return_currency,
      asNumber(row.return_unit_price),
      null,
      null,
      null,
      row.exception_currency,
      asNumber(row.exception_unit_price),
      null,
      null,
      null,
      null,
      serviceRateMap.get(`${row.version_id}|${row.month_id}|${row.route_id}`),
      row.event_note,
      row.management_action_note,
      row.scenario_note,
      row.capacity_assumption,
    ];
  });
  writeMatrix(sheet, 4, 1, [headers, ...rows]);
  ...
  setFormulaColumn(
    ...
    "N",
    modelCalcStart,
    formulaRows.map((r) => `=J${r}*K${r}*L${r}*M${r}`),
  );
  setFormulaColumn(
    sheet,
    "P",
    modelCalcStart,
    formulaRows.map((r) => `=N${r}*O${r}`),
  );
  setFormulaColumn(
    sheet,
    "R",
    modelCalcStart,
    formulaRows.map((r) => `=P${r}*Q${r}`),
  );
  setFormulaColumn(
    sheet,
    "T",
    modelCalcStart,
    formulaRows.map((r) => `=N${r}*S${r}`),
  );
  setFormulaColumn(
    sheet,
    "V",
    modelCalcStart,
    formulaRows.map((r) => `=P${r}*U${r}`),
  );
  setFormulaColumn(
    sheet,
    "AC",
    modelCalcStart,
    formulaRows.map((r) => `=IF(W${r}=\"EUR\",AA${r},IF(W${r}=\"GBP\",AB${r},1))`),
  );
  setFormulaColumn(
    sheet,
    "AD",
    modelCalcStart,
    formulaRows.map((r) => `=R${r}*X${r}`),
  );
  setFormulaColumn(
    sheet,
    "AE",
    ...
    formulaRows.map(
      (r) =>
        `=IF(Y${r}=\"PERCENTAGE_OF_BASE_FREIGHT\",AD${r}*Z${r},IF(Y${r}=\"PER_CHARGEABLE_KG\",R${r}*Z${r},P${r}*Z${r}))`,
    ),
  );
  setFormulaColumn(
    sheet,
    "AF",
    modelCalcStart,
    formulaRows.map((r) => `=AD${r}*AC${r}`),
  );
  setFormulaColumn(
    sheet,
    "AG",
    modelCalcStart,
    formulaRows.map((r) => `=AE${r}*AC${r}`),
  );
  setFormulaColumn(
    sheet,
    "AJ",
    modelCalcStart,
    formulaRows.map((r) => `=IF(AH${r}=\"EUR\",AA${r},IF(AH${r}=\"GBP\",AB${r},1))`),
  );
  setFormulaColumn(
    sheet,
    "AK",
    modelCalcStart,
    formulaRows.map((r) => `=N${r}*AI${r}`),
  );
  setFormulaColumn(
    sheet,
    "AL",
    modelCalcStart,
    formulaRows.map((r) => `=AK${r}*AJ${r}`),
  );
  setFormulaColumn(
    sheet,
    "AO",
    modelCalcStart,
    formulaRows.map((r) => `=IF(AM${r}=\"EUR\",AA${r},IF(AM${r}=\"GBP\",AB${r},1))`),
  );
  setFormulaColumn(
    sheet,
    "AP",
    modelCalcStart,
    formulaRows.map((r) => `=P${r}*AN${r}`),
  );
  setFormulaColumn(
    sheet,
    "AQ",
    modelCalcStart,
    formulaRows.map((r) => `=AP${r}*AO${r}`),
  );
  setFormulaColumn(
    sheet,
    "AT",
    modelCalcStart,
    formulaRows.map((r) => `=IF(AR${r}=\"EUR\",AA${r},IF(AR${r}=\"GBP\",AB${r},1))`),
  );
  setFormulaColumn(
    sheet,
    "AU",
    modelCalcStart,
    formulaRows.map((r) => `=T${r}*AS${r}`),
  );
  setFormulaColumn(
    sheet,
    "AV",
    modelCalcStart,
    formulaRows.map((r) => `=AU${r}*AT${r}`),
  );
  setFormulaColumn(
    sheet,
    "AY",
    modelCalcStart,
    formulaRows.map((r) => `=IF(AW${r}=\"EUR\",AA${r},IF(AW${r}=\"GBP\",AB${r},1))`),
  );
  setFormulaColumn(
    sheet,
    "AZ",
    modelCalcStart,
    formulaRows.map((r) => `=V${r}*AX${r}`),
  );
  setFormulaColumn(
    sheet,
    "BA",
    modelCalcStart,
    formulaRows.map((r) => `=AZ${r}*AY${r}`),
  );
  setFormulaColumn(
    sheet,
    "BB",
    modelCalcStart,
    formulaRows.map((r) => `=AF${r}+AG${r}+AL${r}+AQ${r}+AV${r}+BA${r}`),
  );
...
const costEnd = costStart + data.cost_facts.length - 1;
{
  ...
  titleBand(
    sheet,
    "履约变动成本事实",
    "每条成本事实与 Model_Calc 的线路公式列直接勾稽；Actual 权威金额差异必须为零。",
    "T",
  );
  const headers = [
    "Version",
  ...
  const componentColumn = {
    BASE_FREIGHT: "AF",
    FUEL_SURCHARGE: "AG",
    FRONTLINE_VARIABLE_LABOR: "AL",
    PACKAGING: "AQ",
    RETURN_LOGISTICS: "AV",
    BILLABLE_EXCEPTION: "BA",
  };
  const rows = data.cost_facts.map((row) => [
    ...
    row.currency,
    asNumber(row.original_amount),
    asNumber(row.fx_rate),
    row.model_cost_cny_high_precision,
    asNumber(row.model_cost_cny_report),
    asNumber(row.authority_cost_cny_report),
    asNumber(row.authority_model_delta_cny),
    row.posting_basis,
    null,
    null,
  ]);
  ...
  setFormulaColumn(
    ...
    Array.from(
      { length: rows.length },
      (_, index) => `=ROUND(S${costStart + index}-O${costStart + index},4)`,
    ),
  );
...
const fixedEnd = fixedStart + data.fixed_cost_facts.length - 1;
{
  ...
  titleBand(
    sheet,
    "固定成本事实",
    "仓租、一线作业基础人工、仓库管理人工和系统费用按发货仓或共享范围单独展示，不分摊到目的国或线路。",
    "M",
  );
  const headers = [
    "Version",
  ...
  const rows = data.fixed_cost_facts.map((row) => [
    row.version_id,
    row.data_type,
    row.scenario_id,
    monthDate(row.month_id),
  ...
  setFormulaColumn(
    ...
    "L",
    fixedStart,
    Array.from({ length: rows.length }, (_, i) => `=H${fixedStart + i}*I${fixedStart + i}`),
  );
  setFormulaColumn(
    sheet,
    "M",
    fixedStart,
    Array.from({ length: rows.length }, (_, i) => `=L${fixedStart + i}-K${fixedStart + i}`),
  );
  styleHeader(sheet, "A4:M4");
  styleBody(sheet, `A5:M${fixedEnd}`);
...
const gmvEnd = gmvStart + data.gmv_facts.length - 1;
{
  ...
  const rows = data.gmv_facts.map((row) => [
    row.version_id,
    row.data_type,
    row.scenario_id,
    monthDate(row.month_id),
  ...
  setFormulaColumn(
    ...
    "K",
    gmvStart,
    Array.from({ length: rows.length }, (_, i) => `=G${gmvStart + i}*H${gmvStart + i}`),
  );
  setFormulaColumn(
    sheet,
    "L",
    gmvStart,
    Array.from({ length: rows.length }, (_, i) => `=K${gmvStart + i}-J${gmvStart + i}`),
  );
  styleHeader(sheet, "A4:N4");
  styleBody(sheet, `A5:N${gmvEnd}`);
...
const serviceEnd = serviceStart + data.service_facts.length - 1;
{
  ...
  titleBand(
    sheet,
    "服务事实与成熟度",
    "Actual 准时率 = 按时妥投包裹 ÷ 已到承诺截止日期包裹；Forecast/情景使用版本化线路预计准时率。",
    "P",
  );
  const headers = [
    "Version",
  ...
  const rows = data.service_facts.map((row) => [
    row.version_id,
    row.data_type,
    row.scenario_id,
    monthDate(row.month_id),
    ...
    row.carrier_id,
    row.transport_mode_id,
    asNumber(row.cohort_package_count),
    asNumber(row.due_package_count),
    asNumber(row.on_time_package_count),
    asNumber(row.on_time_rate),
    asNumber(row.service_maturity_rate),
    null,
    null,
  ]);
  writeMatrix(sheet, 4, 1, [headers, ...rows]);
  setFormulaColumn(
    ...
    Array.from(
      { length: rows.length },
      (_, i) =>
        `=IF(K${serviceStart + i}=\"\",M${serviceStart + i},L${serviceStart + i}/K${serviceStart + i})`,
    ),
  );
  setFormulaColumn(
    sheet,
    "P",
    serviceStart,
    Array.from({ length: rows.length }, (_, i) => `=O${serviceStart + i}-M${serviceStart + i}`),
  );
  styleHeader(sheet, "A4:P4");
  styleBody(sheet, `A5:P${serviceEnd}`);
...
const monthlyRowMap = sourceRowMap(
  data.monthly_summary,
  (row) => `${row.series_id}|${row.month_id}`,
  monthlyStart,
);
{
  ...
  titleBand(
    sheet,
    "月度管理汇总",
    "统一时间轴；Actual、Budget、Forecast、Latest Outlook 与三类情景使用同一指标口径。",
    "P",
  );
  const headers = [
    "Series",
    ...
    "Variable Cost CNY",
    "Fixed Cost CNY",
    "Source Total Cost CNY",
    "Recalc Total Cost CNY",
    "Delta",
    "GMV CNY",
    "Company Total Cost Rate",
    "Unit Variable/Order",
    "Transport Cost/kg",
    "On-time Rate",
    "Maturity Rate",
  ];
  const rows = data.monthly_summary.map((row) => [
    row.series_id,
    monthDate(row.month_id),
    asNumber(row.orders),
    asNumber(row.packages),
    asNumber(row.chargeable_weight_kg),
    asNumber(row.variable_cost_cny_report),
    asNumber(row.fixed_cost_cny_report),
    ...
    null,
    null,
    asNumber(row.transport_cost_per_kg_cny),
    asNumber(row.on_time_rate),
    asNumber(row.service_maturity_rate),
  ]);
  writeMatrix(sheet, 4, 1, [headers, ...rows]);
  setFormulaColumn(
    ...
    "I",
    monthlyStart,
    Array.from({ length: rows.length }, (_, i) => `=F${monthlyStart + i}+G${monthlyStart + i}`),
  );
  setFormulaColumn(
    sheet,
    "J",
    ...
    Array.from(
      { length: rows.length },
      (_, i) => `=ROUND(I${monthlyStart + i}-H${monthlyStart + i},4)`,
    ),
  );
  setFormulaColumn(
    sheet,
    "L",
    monthlyStart,
    Array.from({ length: rows.length }, (_, i) => `=I${monthlyStart + i}/K${monthlyStart + i}`),
  );
  setFormulaColumn(
    sheet,
    "M",
    monthlyStart,
    Array.from({ length: rows.length }, (_, i) => `=F${monthlyStart + i}/C${monthlyStart + i}`),
  );
  styleHeader(sheet, "A4:P4");
  styleBody(sheet, `A5:P${monthlyEnd}`);
...
const countryEnd = countryStart + data.country_summary.length - 1;
{
  ...
  const rows = data.country_summary.map((row) => [
    row.series_id,
    monthDate(row.month_id),
    row.destination_country_id,
    asNumber(row.orders),
    ...
    asNumber(row.variable_cost_cny_report),
    asNumber(row.gmv_cny_report),
    asNumber(row.destination_variable_cost_rate),
    asNumber(row.air_package_share),
    asNumber(row.carrier_c_package_share),
    asNumber(row.on_time_rate),
    asNumber(row.service_maturity_rate),
    null,
    row.destination_country_id === "GB" && row.month_id === "2026-08" ? "核心异常范围" : null,
  ]);
  writeMatrix(sheet, 4, 1, [headers, ...rows]);
  setFormulaColumn(
    sheet,
    "M",
    countryStart,
    Array.from({ length: rows.length }, (_, i) => `=F${countryStart + i}/D${countryStart + i}`),
  );
  styleHeader(sheet, "A4:N4");
  styleBody(sheet, `A5:N${countryEnd}`);
...
attributionRows.sort((a, b) =>
  `${a.comparison_id}|${a.month_id}|${a.method_id}|${a.factor_id}|${a.destination_country_id}`.localeCompare(
    `${b.comparison_id}|${b.month_id}|${b.method_id}|${b.factor_id}|${b.destination_country_id}`,
  ),
);
...
const attributionEnd = attributionStart + attributionRows.length - 1;
{
  ...
  titleBand(
    sheet,
    "五因素归因与 Shapley 参考",
    "正式口径为量→结构→效率→价→汇率连环替代；ALL 为公司层，国家行由同一原子归因向上汇总。",
    "S",
  );
  const headers = [
    "Comparison",
  ...
  const rows = attributionRows.map((row) => [
    ...
    asNumber(row.amount_cny_report),
    asNumber(row.shapley_difference_cny),
    asNumber(row.normalized_order_sensitivity),
    row.is_order_sensitive ?? null,
  ]);
  writeMatrix(sheet, 4, 1, [headers, ...rows]);
  styleHeader(sheet, "A4:J4");
  styleBody(sheet, `A5:J${attributionEnd}`);
  ...
  const reconRows = data.attribution_reconciliation.map((row, index) => {
    ...
    return [
      row.comparison_id,
      monthDate(row.month_id),
      asNumber(row.expected_variable_cost_difference_cny),
      asNumber(row.chain_total_cny),
      asNumber(row.chain_delta_cny),
      asNumber(row.shapley_total_cny),
      asNumber(row.shapley_delta_cny),
      null,
      null,
    ];
  });
  ...
  setFormulaColumn(
    ...
    "R",
    5,
    reconRows.map((_, i) => `=IF(ABS(O${5 + i})<0.0001,\"PASS\",\"FAIL\")`),
  );
  setFormulaColumn(
    sheet,
    "S",
    5,
    reconRows.map((_, i) => `=IF(ABS(Q${5 + i})<0.0001,\"PASS\",\"FAIL\")`),
  );
  styleHeader(sheet, "K4:S4");
  styleBody(sheet, `K5:S${4 + reconRows.length}`);
  ...
  sheet.getRange(`R5:S${4 + reconRows.length}`).conditionalFormats.add("containsText", {
    text: "PASS",
...
// Scenarios
{
  ...
  titleBand(
    sheet,
    "情景结果与实施节奏",
    "Base 沿用正式 Forecast；Growth 为英国订单 +10%；Cost Saving 只调整现有线路，服务阈值为软提示。",
    "K",
  );
  const headers = [
    "Scenario",
  ...
  for (const series of scenarioSeries) {
    for (let month = 9; month <= 12; month += 1) {
      ...
      rows.push([
        ...
        null,
        null,
        "UNVERIFIED_UNLIMITED_FOR_MVP",
        series === "SCENARIO_COST_SAVING"
          ? month === 9
            ? "准备期"
            : month === 10
              ? "部分调整"
              : "目标结构"
          : series === "SCENARIO_GROWTH"
            ? "各月相对 Forecast +10%，不复利"
            : "沿用正式 Forecast",
      ]);
    }
  }
  ...
  sheet.getRange("F5:F16").conditionalFormats.add("cellIs", {
    operator: "lessThan",
    formula: 0,
    format: { fill: COLORS.greenLight, font: { color: COLORS.green } },
  });
  sheet.getRange("I5:I16").conditionalFormats.add("cellIs", {
    operator: "lessThan",
    formula: 0,
    format: { fill: COLORS.orangeLight, font: { color: COLORS.orange } },
  });
  sheet.freezePanes.freezeRows(4);
  sheet.freezePanes.freezeColumns(2);
...
// Evaluation
{
  ...
  titleBand(
    sheet,
    "固定中文 AI 评估集",
    "20 题；至少通过 18 题；两道越界题必须全部通过；数字必须精确一致。",
    "G",
  );
  const headers = [
    "ID",
  ...
  const rows = evaluation.questions.map((row) => [
    ...
    row.question,
    row.standard_answer,
    JSON.stringify(row.required_numbers, null, 0),
    row.evidence.join(" | "),
    row.hard_guardrail ?? null,
  ]);
  writeMatrix(sheet, 4, 1, [headers, ...rows]);
  styleHeader(sheet, "A4:G4");
  styleBody(sheet, "A5:G24");
...
// Checks
{
  ...
  titleBand(
    sheet,
    "模型检查与发布状态",
    "PASS 仅表示本工作簿内全部必要检查通过；不代表虚构参数来自真实企业。",
    "E",
  );
  ...
  sheet.getRange("A5:E6").format = {
    fill: COLORS.greenLight,
    font: { bold: true, color: COLORS.green, size: 22, name: "Aptos Display" },
    horizontalAlignment: "center",
    verticalAlignment: "center",
  ...
  const checks = [
    [
      "Cost facts vs Model_Calc row tie-out",
      `=COUNTIF('Cost_Facts'!$T$${costStart}:$T$${costEnd},\">0.0500\")+COUNTIF('Cost_Facts'!$T$${costStart}:$T$${costEnd},\"<-0.0500\")`,
      "0 rows",
      "Cost_Facts / Model_Calc",
      '=IF(B9=0,"PASS","FAIL")',
    ],
    [
      "Actual authority amount delta",
      `=COUNTIFS('Cost_Facts'!$B$${costStart}:$B$${costEnd},\"ACTUAL\",'Cost_Facts'!$Q$${costStart}:$Q$${costEnd},\"<>0\")`,
      "0 rows",
      "Actual source layer",
      '=IF(B10=0,"PASS","FAIL")',
    ],
    [
      "Fixed cost recalculation",
      `=COUNTIF('Fixed_Costs'!$M$${fixedStart}:$M$${fixedEnd},\">0.0500\")+COUNTIF('Fixed_Costs'!$M$${fixedStart}:$M$${fixedEnd},\"<-0.0500\")`,
      "0 rows",
      "Fixed_Costs",
      '=IF(B11=0,"PASS","FAIL")',
    ],
    [
      "GMV recalculation",
      `=COUNTIF('GMV_Facts'!$L$${gmvStart}:$L$${gmvEnd},\">0.0500\")+COUNTIF('GMV_Facts'!$L$${gmvStart}:$L$${gmvEnd},\"<-0.0500\")`,
      "0 rows",
      "GMV_Facts",
      '=IF(B12=0,"PASS","FAIL")',
    ],
    [
      "Service rate recalculation",
      `=COUNTIF('Service_Facts'!$P$${serviceStart}:$P$${serviceEnd},\">0.000001\")+COUNTIF('Service_Facts'!$P$${serviceStart}:$P$${serviceEnd},\"<-0.000001\")`,
      "0 rows",
      "Service_Facts",
      '=IF(B13=0,"PASS","FAIL")',
    ],
    [
      "Monthly total = variable + fixed",
      `=COUNTIF('Monthly_Summary'!$J$${monthlyStart}:$J$${monthlyEnd},\">0.0500\")+COUNTIF('Monthly_Summary'!$J$${monthlyStart}:$J$${monthlyEnd},\"<-0.0500\")`,
      "0 rows",
      "Monthly_Summary",
      '=IF(B14=0,"PASS","FAIL")',
    ],
    [
      "Latest Outlook composition",
      `=ROUND(SUMIF('Monthly_Summary'!$A$${monthlyStart}:$A$${monthlyEnd},\"LATEST_OUTLOOK\",'Monthly_Summary'!$I$${monthlyStart}:$I$${monthlyEnd})-SUMIF('Monthly_Summary'!$A$${monthlyStart}:$A$${monthlyEnd},\"ACTUAL\",'Monthly_Summary'!$I$${monthlyStart}:$I$${monthlyEnd})-SUMIF('Monthly_Summary'!$A$${monthlyStart}:$A$${monthlyEnd},\"FORECAST\",'Monthly_Summary'!$I$${monthlyStart}:$I$${monthlyEnd}),4)`,
      "±0.05 CNY",
      "Monthly_Summary",
      '=IF(ABS(B15)<=0.05,"PASS","FAIL")',
    ],
    [
      "Chain attribution reconciliation",
      `=COUNTIF('Attribution'!$R$5:$R$${4 + data.attribution_reconciliation.length},\"FAIL\")`,
      "0 rows",
      "Attribution reconciliation",
      '=IF(B16=0,"PASS","FAIL")',
    ],
    [
      "Shapley attribution reconciliation",
      `=COUNTIF('Attribution'!$S$5:$S$${4 + data.attribution_reconciliation.length},\"FAIL\")`,
      "0 rows",
      "Attribution reconciliation",
      '=IF(B17=0,"PASS","FAIL")',
    ],
    [
      ...
      "=COUNTA('Evaluation'!$A$5:$A$24)",
      "20",
      "Evaluation",
      '=IF(B18=20,"PASS","FAIL")',
    ],
    [
      "Boundary question count",
      "=COUNTIF('Evaluation'!$B$5:$B$24,\"数据不足与越界保护\")",
      "2",
      "Evaluation",
      '=IF(B19=2,"PASS","FAIL")',
    ],
  ...
  writeMatrix(
    ...
    9,
    1,
    checks.map((row) => [row[0], null, row[2], row[3], null]),
  );
  ...
  sheet.getRange("E9:E19").conditionalFormats.add("containsText", {
    text: "PASS",
    format: { fill: COLORS.greenLight, font: { color: COLORS.green, bold: true } },
  });
  sheet.getRange("E9:E19").conditionalFormats.add("containsText", {
...
// Sources
{
  ...
  titleBand(
    sheet,
    "来源、版本与刷新记录",
    "所有业务数据均为虚构演示数据；来源字段用于可审计性，不代表真实企业系统。",
    "J",
  );
  const headers = [
    "Item",
  ...
  const rows = [
    ...
    [
      ...
      "CNY / operational units",
      "2026 full year",
      "Synthetic generated data",
      "logiplan-2026-demo-data.json",
      "data/generated/logiplan-2026-demo-data.json",
      "Portfolio project",
      "高精度 Decimal 权威数据",
      "2026-08-14",
    ],
    [
      ...
      "ai-evaluation-baseline.json",
      "data/generated/ai-evaluation-baseline.json",
      "Portfolio project",
      "标准答案与证据范围",
      "2026-08-14",
    ],
    [
      ...
      "xlsx",
      "2026-08 close",
      "Generated audit workbook",
      "LogiPlan-AI-2026-Demo-Data.xlsx",
      "outputs/2026-demo-data-current-labor-split",
      "Portfolio project",
      "公式审阅与展示层",
      "2026-08-14",
    ],
...
const renderRanges = {
  ...
  Fixed_Costs: "A1:M20",
  GMV_Facts: "A1:N20",
  Service_Facts: "A1:P20",
  Monthly_Summary: "A1:P30",
  Country_Summary: "A1:N25",
  Attribution: "A1:S30",
  Scenarios: "A1:K16",
  Evaluation: "A1:G12",
  Checks: "A1:E19",
